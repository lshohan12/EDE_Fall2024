...
date: "Fall 2024"
output: html_document #changed from pdf to html for intearctive
runtime: shiny
editor_options: 
...
my_plot <- nutrient_data %>% 
  ...
  labs(
    ...
    shape = "Lake", 
    color = "Depth (m)") + #changed from fill = and it changed legend depth to Depth (m)
...
my_plot <- nutrient_data %>% 
  ...
  ggplot(aes(
    x = sampledate,
    y = .data[[the_parameter]], #overriding ggplot set up that allows us to not have to '' everything in aes - so now it knows its a column
    #same thing -> y = !!sym(the_pararmeter)
    color = depth,
    shape = lakename)) +
  geom_point(alpha = 0.4, size = 2) +
  scale_x_date(
...
#function - {} surround whole code chunk after parameters are set
...
plot_it('nh34', 1994, 2020) #do in console too
