...
PMplot <- 
  ...
  geom_point() #as.factor - assigns shapes to different years
print(PMplot)
...
PMplot.faceted <-
  ...
  geom_point() +
  facet_wrap(vars(Site.Name), nrow = 3) #creates new plot for each site, and creates 3 rows of them 
...
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
                  Site.Name == "Bryson City"), #subset just pulls certain ones 
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) #Site name by year - very cool
...
lightvsDO <- 
  ...
  geom_point()
print(lightvsDO) #ugly bc of outliers
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() #want o on top and larger numbers on bottoms- bc we are doing depth 
print(tempvsdepth)

NvsP <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth)) +
  geom_point() +
  geom_smooth(method = lm) + #linear line that plots trend line between two nutrients
  geom_abline(aes(slope = 16, intercept = 0)) #if you know slope and intercept 
print(NvsP)
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? - fill changes the fill of the box vs the whole box and dots for each category
print(Nutrientplot3)
...
print(Nutrientplot4) #can see a little more vs other one you cant see all the box plots

# Violin plot
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_violin(aes(color = nutrient)) #similar to box plots but shows density plot of category 
print(Nutrientplot5)

# Frequency polygons
# Using a tidy dataset
    #not a great graph at all 
Nutrientplot6 <-
...
# Using a gathered dataset - cleaner way than above!!!
Nutrientplot7 <-   
