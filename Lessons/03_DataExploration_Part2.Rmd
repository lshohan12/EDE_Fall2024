...
str(USGS.flow.data) #all factors and nums so good

# Check our date column: is it a date object?
class(USGS.flow.data$datetime) # yes its a factor so we want to change
...
ggplot(data = USGS.flow.data , aes(x = discharge.mean.approval)) + #aesethics x axis 
  geom_bar() #shows how we want to plot geombar - creates bar plot
```
...
```{r, fig.height = 3, fig.width = 4}
# y is count of numbers in grouping 
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean))
#same as ggplot(USGS.flow.data, aes(x = discharge.mean)) + geom_histogram()

# binwidth = 10 units of charge - gives finer plot
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

# bins and binwidth have nothing to do with the actual data
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

# scale xx continuous - allows us to define extent of xaxis
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
  geom_histogram(binwidth = 10) + 
  scale_x_continuous(limits = c(0, 500))
  
# histogram of continuos data duh
ggplot(USGS.flow.data) +
...
#
ggplot(USGS.flow.data) +
  geom_freqpoly(aes(x = gage.height.mean), bins = 50) + #allows to compare dif distributions
  geom_freqpoly(aes(x = gage.height.min), bins = 50, color = "darkgray") + #shows 2nd distribution line
  geom_freqpoly(aes(x = gage.height.max), bins = 50,  lty = 2) + #lty - line type - dashed
  scale_x_continuous(limits = c(0, 10))

#color inside aes - apply data values to plot 
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#x is categorical y is continous 
#fat line is mean - boxes are the ranges of above or below - dots above are outliers out of IQR - easy to compare data
ggplot(USGS.flow.data) +
...
#violin gives us width is porportional to # of values
ggplot(USGS.flow.data) +
