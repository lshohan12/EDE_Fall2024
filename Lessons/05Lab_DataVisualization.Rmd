...
ggplot(data=data.frame(x=0,y=0))+
  geom_point(aes(x=x,y=y)) + #this line and above plots one point 
  geom_text(
    x=0,
    y=0.025, #adding text a little above original point moving Y axis wise
    label=expression(paste("Mean is ", mu,",", R^{2},'=0.6'))
    )  #expression funct - pasting all these characters together - mu will be converted to greek letter under expression function

#Print the plot
print(Plot_text)

#Save the plot - saves somewhere and can download and save
ggsave(
...
#List unique values
unique(PeterPaul.chem.nutrients$month) #5,6,7,8,9,10,11,2

#Plot how many Lake observations occur each month
PeterPaul.chem.nutrients %>%
  ggplot() +
  geom_bar(aes(x = month)) #will give frequency - what will be x axis? variable u want to plot freq for
...
PeterPaul.chem.nutrients$month <- factor(PeterPaul.chem.nutrients$month,
       ...
       labels = month.abb) #month.abb - r variable that has jan - dec variables
...
PeterPaul.chem.nutrients %>%
  ggplot() +
  geom_bar(aes(x = month)) +
  scale_x_discrete(drop = F) #makes sure missing levels arent dropped
...
surface_temp_1999_plot <- PeterPaul.chem.nutrients %>% #assigning it into variable
...
# Change the ylab name - yaxis title 

surface_temp_by_day_plot <- PeterPaul.chem.nutrients %>%
  filter(depth == 0) %>%
  ggplot( #once you start ggplot use +
    mapping = aes( #defining aes in ggplot defines all layers
      x=daynum,
      y=temperature_C,
      color=year4) #color inside aes - color coding plot based on that column 
    ) + 
  geom_point() +
  facet_wrap(facets = vars(lakename), nrow = 2) + #ncol = 2
  ylab('Temperature [C]') +
  labs(color = 'Year')
...
temperature_by_date_depth <- PeterPaul.chem.nutrients %>%
  ggplot(aes(
      x = sampledate,
      y = temperature_C,
      color = depth)) +
  geom_point(size = 0.5) +
  scale_color_viridis(option = 'magma', direction = -1) + #better representation of temperature
  scale_x_date(date_breaks = '5 years', date_labels = '%Y') #scale x date funct just understands date object
temperature_by_date_depth
