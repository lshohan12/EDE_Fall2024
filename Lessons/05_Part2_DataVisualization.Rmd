...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + #lty will be a straight line 
  geom_hline(yintercept = 100, lty = 2) +
  geom_point(alpha = 0.5, size = 1.5) + #alpha is transparency 
  geom_text(x = as.Date("2020-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") +   # hjust = 1 means left aligning text 
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), #limits to just plot certain data
    date_breaks = "2 months", date_labels = "%b %y") + #date breaks into 2 months 
...
plot_grid(NvsP2, Nutrientplot, nrow = 2, align = 'h', rel_heights = c(1.25, 1))
 #rel heights - adjusts heights of plots so they are on same grid
