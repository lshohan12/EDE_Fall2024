...
library(lubridate) #creating date objects
NTL.phys.data <- read.csv("./Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv", stringsAsFactors = TRUE) #Stringsfactors = true works for character entries

colnames(NTL.phys.data)
head(NTL.phys.data)
summary(NTL.phys.data)
str(NTL.phys.data) #shows sample date is a factor 
dim(NTL.phys.data)

class(NTL.phys.data$sampledate)
# Format sampledate as date
NTL.phys.data$sampledate <- as.Date(NTL.phys.data$sampledate, format = "%m/%d/%y") #column u want to convert, format that it is in
```
...
```{r, results = "hide"}
vignette("dplyr") #three categories of rows, columns, and groups of rows!!!
#cheat sheet of three 
```
...
class(NTL.phys.data$depth) #numeric 
   #can only compare same types with one another 

# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] #filtering only rows with 0 on depth col

# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0) #datset, what ur trynna filter
...
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") #or 
NTL.phys.data.PeterPaul2 <- filter(NTL.phys.data, lakename != "Central Long Lake" & 
                       ...
                       #strategy is excluding the lakes you don't want!!

NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) 
        #%in% - useful with more than 2 elements, using vector after that !!! easier way to do it then with or 

# Choose a range of conditions of a numeric or integer variable
summary(NTL.phys.data$daynum)  #152 first day of June and 304 is Nov1 
NTL.phys.data.JunethruOctober1 <- filter(NTL.phys.data, daynum > 151 & daynum < 305) 
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305) #can use comma and it leads to same thing
NTL.phys.data.JunethruOctober3 <- filter(NTL.phys.data, daynum >= 152 & daynum <= 304) #numbers included
NTL.phys.data.JunethruOctober4 <- filter(NTL.phys.data, daynum %in% c(152:304)) #%in% functs!!!- when it matches the vector itll be extracted 
...
NTL.phys.data.depth.descending <- arrange(NTL.phys.data, desc(depth)) #in descending!! so high to low
...
```
### Select - NOW IN COLUMNS

Selecting allows us to choose certain columns (variables) in our dataset.

```{r selecting columns}
NTL.phys.data.temps <- select(NTL.phys.data, lakename, sampledate:temperature_C) #want lakename -> --- creating subset with all columns in between sample date : temperature 
...
#add column in temperature in fahernheit - new column to existing data set 
#temperature_F is new column name
...
# add a month column to the dataset
    #extracts month from date object - in this we have new column
...
NTL.phys.data.PeterPaul1 <- select(NTL.phys.data.PeterPaul1, lakeid:daynum, month, sampledate:comments)
        #columns before, the column u want moved, columns after

# find out the start and end dates of the dataset
    #interval object with specificed start and end date
interval(NTL.phys.data.PeterPaul1$sampledate[1], NTL.phys.data.PeterPaul1$sampledate[21613]) #start and end dates can find the last number on obs. dimensions
interval(first(NTL.phys.data.PeterPaul1$sampledate), last(NTL.phys.data.PeterPaul1$sampledate)) #another way to calculate it  
    #better way!!!! because it will work on any dataframe regardless on order
```
