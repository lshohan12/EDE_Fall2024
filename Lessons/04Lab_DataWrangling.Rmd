...
## Set up your session
SHORT CUTS - FROM SLIDESHOW
<option> + -  results in <-
shift + ctrl + m : %>%
shift + option + I : new code chunk
...
glimpse(NTL.phys.data)#$sampledate) #looking at structure of data - tidyverse 
...
```{r filter data}
# note the data types of these two columns - filtering works definitely depending on class
class(NTL.phys.data$lakeid) #factor
class(NTL.phys.data$depth) #numeric

# dplyr filtering - doing a numeric
NTL.phys.data.surface <- filter(NTL.phys.data, depth == 0) 

# Choose multiple conditions to filter - with factors you can use == but choosing multiple otptions we use %in% - goes through every row and see if its in this list and if its not it is skipped
...
NTL.phys.data.JunethruOctober <- filter(NTL.phys.data, daynum %in% c(152:304)) #using a slice - 152 -june starts and october ends in 304 
```
...
NTL.phys.data %>% filter(year(sampledate) == 1999)
...
NTL.Tuesday.90.99 <- filter(NTL.phys.data, year4 %in% c(1990:1999) & lakename == 'Tuesday Lake')
nrow(NTL.Tuesday.90.99)


```
Question: Why don't we filter using row numbers?

> Answer: want to filter on data itself not how its organized
...
NTL.phys.data.processed <- 
  ...
  select(lakename, sampledate:temperature_C) %>% #subsetting my columns 
  mutate(temperature_F = (temperature_C*9/5) + 32) #calculating new column in dataset

#Exercise 3: Using a pipe filter NTL.phys.data for Tuesday Lake from 1990 
# through 1999 only for July.

NTL.phys.data.Tuesday.90.99.July <-
  NTL.phys.data %>%
  filter(lakename == 'Tuesday Lake') %>% 
  filter(year4 %in% c(1990:1999)) %>%
  filter(month(sampledate) == 7)
 
nrow(NTL.phys.data.Tuesday.90.99.July) 
