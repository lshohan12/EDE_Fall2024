...
NTL.nutrient.data.PeterPaul <- 
  ...
  select(lakeid:daynum, month, sampledate:comments) %>% #select all the columns to reorder month column
  drop_na(depth) #dropping NAs
...
gather / pivot_longer: transforming cols into rows - make data longer
spread / pivot_wider: rows into cols - make data wider
...
NTL.nutrient.data.PeterPaul.gathered <- pivot_longer(NTL.nutrient.data.PeterPaul.subset,tn_ug:po4, names_to = "nutrient", values_to = "concentration") #spread into two columns with names and vales - more rows

NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration)) #getting rid of NAs on the new longer dataset
count(NTL.nutrient.data.PeterPaul.gathered, nutrient) #will return num of rows with just count, but specifying column will count by nutrients
...
LitterFiles = list.files(path = "./Data/Raw/NIWO_Litter/", pattern="*.csv", full.names=TRUE) #list files() - full names of files - read all files in this folder
LitterFiles

Litter <- LitterFiles %>%
  ldply(read.csv) #she didnt have plyr::
```
...
#Run incrementally to find the bug
NTL.PeterPaul.summaries <- 
  NTL.phys.nutrient.data.PeterPaul %>%
  filter(depth == 0) %>%
  group_by(lakename, month) %>%
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>% #and to get no NAs
