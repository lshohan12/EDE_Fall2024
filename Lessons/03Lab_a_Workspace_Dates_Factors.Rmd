...
filename <- here('Data','Raw','USGS_Site02085000_Flow_Raw.csv') #instead of , can do / aka paths
print(filename)
...
filenames <- dir(here('Data','Raw'),pattern = ("*.csv")) #dir command - looks in data raw folder with everything with this pattern so all csv files
print(filenames)
```
...
```{r DateExercise1}
#install.packages("lubridate") --- always need to comment out install demands for knitting and in general
library(lubridate)
...
date_juneteenth #spits out 1865-06-19
```
### The century issue
...
```{r DateExercise2}
#century issue - its giving 2055 'future data' when it should be 1955
str_past <- "55-feb-3"
...
fix.early.dates <- function(d, cutoff) {
       ...
       # year(d) by 100 and returns the remainder 
           #gonna take the year of the date object which is 2055 - and then it does modular division and just
          #gives last bit of year so it returns 55 and strips off century
       
       year(d) <- ifelse(m > cutoff, 1900+m, 2000+m)  #true then false
       # This will update year(d), year() is the lubridate function that  
       # returns the year for a date object
            #takes the year cutoff of 55 for example - 
       return(d)
}
...
```{r centuryfix}
#Fix for century issue - more shorter ways to fix dates
str_past <- "55-feb-3"

#Alternative 1
date_obj_past <- fast_strptime(str_past,"%y-%b-%d",cutoff_2000=24L) 
                #L is there so its not treated as floating point, L makes it be read as integer
date_obj_past
...
#Read packages
library(tidyverse);library(lubridate); library(here) #seperate with ;
...
sites$date <- ymd_hms(sites$date) #converting date hms - hour/min/sec in our data file
...
sites$dow <- wday(sites$date) #dow is days of week - extracts the weekday

## What class is the dow column?
class(sites$dow)

#List the unique values in the column
unique(sites$dow) #shows unique values
...
week days arent number - factors are categorical variables and helpful
```{r convert dow column to factor and explore}

#Convert to a factor
sites$dow <- factor(sites$dow) #updates from num to factor and has 3 levels now 
...
#Summary - is it meaningful?
summary(sites$dow) #here are the counts - 1 record on day 3 and 148 on day 4 - so different
                 #info now that its a factor

#Show the levels associated with our factor column
levels(sites$dow) #unique values
```
...
ggplot(sites,aes(x=dow)) +
  geom_bar()  #ggplot - data, aesthetics of how to paint so x axis is dow and geombar gives bar plot
```
...
levels(sites$dow) <- c(levels(sites$dow),"1","2","6","7") #shows empty days of the week by adding levels
...
ggplot(sites,aes(x=dow)) +
  geom_bar() + 
  scale_x_discrete(drop=FALSE)  #adding this line makes it not drop empty factors
...
sites$dow <- factor(sites$dow, levels=c("1","2","3","4","5","6","7")) 
        #Or... as.character(seq(1,7))
            #factor(sites$dow, levels = as.character(seq(1,7))
        #reordoring the levels - but not changing values in dataset
...
usgs_flow_data <- read.csv(
  file = here('Data','Raw','USGS_Site02085000_Flow_Raw.csv'),
  stringsAsFactors = T,
  colClasses = (c('site_no'='factor')) #colClasses how u convert num to factors 
)
