...
kable(summary_PM25, caption = 'Mean Particulates (2.5mm)', col.names = c('County','$\\mu g/m^3$'), digits = 2)
kable(summary_Ozone, caption = 'Mean Ozone', col.names = c('County','$\\mu g/m^3$'), digits = 2)
...
```{r echo=FALSE, warning = FALSE, fig.cap= "2018 Monthly Ozone Levels", fig.height=3, fig.width=7}
epa_data %>%
  filter(Year == 2018) %>%
...
```{r echo = FALSE, warning = FALSE, fig.cap = "2019 Monthly Ozone Levels", fig.height=3, fig.width=7}
epa_data %>%
  filter(Year == 2019) %>%
  ggplot(aes(x = Month, y = Ozone)) +
  geom_boxplot() +
...
## Git Repository
[https://github.com/lshohan12/EDE_Fall2024](https://github.com/lshohan12/EDE_Fall2024)
